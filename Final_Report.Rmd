...
# TO DO
- Add Stringency index citation [Our World in Data].
- Update all references numbers
- Review Discussion
...
The Stringency Index is a measure related to country policies regarding lockdown measures, is based on indicators including school closures, workplace closures, and travel bans [cite]. The index is scaled from 0 to 100, higher values means more strict policies. Over the course of the pandemic, the index varied in each country, the data contain the index overtime, from 2020 to 2022, was retrieved from the Our World in Data platform [cite]. For this study, the Stringency index was aggregate by the mean, to represent the lockdown policy strictness, in average during the pandemic, for each country.
...
For this project a Weighted Least Squares (WLS) [6,7] additive regression model will be used to evaluate the relationship, at country level ($n=165$), between the outcome population fully vaccinated ($Y$) and the independent variables ($X$): cases per population percentage ($X_1$), deaths per population percentage ($X_2$), 2020 GDP per capita ($X_3$) and Stringency index ($X_4$), defined as:
