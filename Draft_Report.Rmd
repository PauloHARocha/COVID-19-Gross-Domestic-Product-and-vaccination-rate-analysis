---
title: "COVID-19: Gross Domestic Product and vaccination rate analysis."
...
```
The cumulative cases overtime shows that Americas region had more cases than Europe until the beginning of 2022, showing a high increase in numbers of new cases in Europe in this period (Figure 2). At February 17th 2022, the Americas region has the highest cumulative deaths (2,585,427), 752,451 more deaths than Europe and 1,830,165 more deaths than South-East Asia.

```{r, echo=FALSE, fig.height=8, fig.width=12}
f1 <- ggplot(who_cases_region, aes(x = Date_reported, y = Cumulative_cases)) +
  geom_line(aes(color = WHO_region))+
  scale_x_continuous(breaks = pretty(who_cases_region$Date_reported, n = 12)) +
  labs(x = "Data reported", y = "Cumulative cases",color='WHO region')

f2 <- ggplot(who_cases_region, aes(x = Date_reported, y = Cumulative_deaths)) +
  geom_line(aes(color = WHO_region)) +
  scale_x_continuous(breaks = pretty(who_cases_region$Date_reported, n = 12)) +
  guides(color="none")+
  labs(x = "Data reported", y = "Cumulative deaths")
...
```
The case-mortality rate was generated based on the daily cumulative deaths divided by the daily cumulative cases. All regions had their highest values in 2020, and Europe had the highest case-mortality rate of all around April 2020 (Figure 3).
...
f1 <- ggplot(who_cases_region, aes(x = Date_reported, y = Case_mortality_rate)) +
  geom_line(aes(color = WHO_region)) +
  scale_x_continuous(breaks = pretty(who_cases_region$Date_reported, n = 12)) +
  labs(x = "Data reported", y = "Case-mortality rate",color='WHO region')
...
```
The World bank data [4] for Gross Domestic Product (GDP) contained 2020 GDP estimates for 185 countries. In terms of mean, Europe region has higher values of GDP than Africa, Eastern Mediterranean and South-East Asia (Figure 6).

```{r, echo=FALSE, warning=FALSE, message=FALSE, results=FALSE}
gdp <- read.csv("API_NY.GDP.PCAP.CD_DS2_en_csv_v2_3632113.csv")
...
For this project a Weighted Least Squares (WLS) [6,7] additive regression model will be used to evaluate the relationship, at country level, between the outcome population fully vaccinated ($Y$) and the independent variables ($X$): cases per population percentage ($X_1$), deaths per population percentage ($X_2$) and 2020 GDP ($X_3$), defined as:

$Y = X\beta + \epsilon =  \beta_0 + X_1\beta_1 + X_2\beta_2 + X_3\beta_3 + \epsilon*$

The WLS model [6,7] assume that there is nonconstant variance in the errors $\epsilon*$, define the weight matrix $W$ where the diagonal $w_i=1/\sigma_i^2$ is a reciprocal of each error variance $\sigma_i^2$, and the estimate is:

$\hat{\beta}_{WLS} = (X^TWX)^{-1}X^TWY$

Each weight is inversely proportional to the error variance, where observations with small error variance will have large weight and the ones with large error variance will have small weight [7]. The model was choosen given the presence of a continuos outcome and also because the WLS model deals with the heteroscedasticity issue that is common when applying Ordinary Least Squares models to complex real-world data.

The model was fitted showing a positive relationship between the outcome and cases per population percentage, and between the outcome and 2020 GDP at statistical significant level at 99% (Table 2).
...
summary(fit.wls)$coefficients %>%
  kbl(caption = "Table 3. WLS model coefficients.") %>%
  kable_classic(full_width = F, html_font = "Cambria")
```

# Sensitivity analysis 

The variables histogram showed some outliers taking into consideration the variables deaths per population percentage and 2020 GDP (Figure 7). The observations with deaths per population percentage value higher than 0.6 and 2020 GPD higher than 100,000 where removed from the data (4 observations), given the high absolute values that may influence the model.
...
summary(fit.wls)$coefficients %>%
  kbl(caption = "Table 4. Updated WLS model coefficients.") %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
...
To evaluate if there is an effect in country vaccination levels given cases, deaths and GDP, the WLS was fitted based on 181 countries and showed that for a unit increase in cases per population percentage represents a 0.602 increase in fully vaccinated population percentage. That result indicated that countries with more cases cases per population tends to have a greater percentage of population fully vaccinated by February 2022. The model also showed that a $10,000 increase in GDP represents a 5.017 increase in fully vaccinated population percentage, indicating how more wealth countries will have a greater percentage of population fully vaccinated by February 2022.
...
# Reference {-}
 ...
 [6] Strutz, T. (2011). Data fitting and uncertainty: A practical introduction to weighted least squares and beyond. Wiesbaden, Germany: Vieweg+ Teubner.
 
 [7] 13.1 - weighted least squares: Stat 501. PennState: Statistics Online Courses. (n.d.). Retrieved February 17, 2022, from https://online.stat.psu.edu/stat501/lesson/13/13.1 
